...
#POST Store Methods
@router.post("/store/", response_model=schemas.StoreCreate, status_code=status.HTTP_201_CREATED)
...
#GET Store Methods
@router.get("/store/get_all", response_model=List[schemas.StoreShow])
...
@router.get("/store/{store_id}", response_model=schemas.StoreShow)
...
@router.put("/store/update/{store_id}", status_code=status.HTTP_202_ACCEPTED)
