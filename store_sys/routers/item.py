...
#POST Item Methods
@router.post("/items/", response_model=schemas.ItemCreate, status_code=status.HTTP_201_CREATED)
def create_item(item: schemas.ItemCreate, db: Session = Depends(get_db)):
    return crud.create_item(db=db, item=item)

#GET Item Methods
@router.get("/items/get_all", response_model=List[schemas.ItemShow])
...
@router.get("/items/{item_id}", response_model=schemas.ItemShow, status_code=status.HTTP_200_OK)
...
#UPDATE Item Methods
@router.put("/items/update/{item_id}", status_code=status.HTTP_202_ACCEPTED)
