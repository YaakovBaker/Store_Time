...
#GET Item Methods
@app.get("/items/get_all", response_model=List[schemas.ItemShow])
...
@app.get("/items/{item_id}", response_model=schemas.ItemShow, status_code=status.HTTP_200_OK)
...
#UPDATE Item Methods
@app.put("/items/update/{item_id}", status_code=status.HTTP_202_ACCEPTED)
...
#GET Store Methods
@app.get("/store/get_all", response_model=List[schemas.StoreShow])
...
@app.get("/store/{store_id}", response_model=schemas.StoreShow)
def get_store(store_id: int, db: Session = Depends(get_db)):
    return crud.get_store(db=db, store_id=store_id)

#DELETE Store Methods
@app.delete("/store/delete/{store_id}")
def delete_store(store_id: int, db: Session = Depends(get_db)):
    crud.delete_store(db=db, store_id=store_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)

#UPDATE Store Methods
@app.put("/store/update/{store_id}", status_code=status.HTTP_202_ACCEPTED)
